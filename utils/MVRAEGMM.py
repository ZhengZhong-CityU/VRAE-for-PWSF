...
class MVRAEGMM(nn.Module):
    def __init__(self, num_Gaussians, x_dim, h_dim, z_dim, rnn_layers):
    ...
    ### we assum a GMM for the observation model
        ...
        self.prior_std=nn.Sequential(nn.Linear(self.h_dim, self.z_dim),nn.Softplus())
        ...
        self.encoder_mean=nn.Sequential(nn.Linear(self.h_dim, self.h_dim),nn.ReLU(),nn.Linear(self.h_dim, self.z_dim))
        self.encoder_std=nn.Sequential(nn.Linear(self.h_dim, self.h_dim),nn.ReLU(),nn.Linear(self.h_dim, self.z_dim),nn.Softplus())
    ...
    def forward(self, x):
        ...
        for t in range(x.shape[-1]):
            ...
            z_t=self.reparameterized_sample(encoder_mean_t, encoder_std_t)
            ...
            ## Computing loss
            kld_loss+=self.kld_loss(encoder_mean_t,encoder_std_t,prior_mean_t,prior_std_t)
    ...
    def reparameterized_sample(self, mean, std):
        z=Variable(torch.randn(mean.shape)).to(device)
        sample_z=z*std+mean
        return sample_z
    ...
    def prediction(self, x, pred_length):
        ...
        for t in range(pred_length):
            ...
            z_t = self.reparameterized_sample(prior_mean_t, prior_std_t)
            phi_z_t = self.phi_z(z_t)
    ...
    def kld_loss(self, encoder_mean, encoder_std, prior_mean, prior_std):
        kld_element =  (2 * torch.log(prior_std) - 2 * torch.log(encoder_std) + 
			(encoder_std.pow(2) + (encoder_mean - prior_mean).pow(2)) /
			prior_std.pow(2) - 1)
...
class MMVRAEGMM_model():
    ...
    @property
    def model_dir(self):
        return 'MVRAEGMM_{}_{}_{}_{}_{}_{}'.format(self.args.train_length, self.args.num_Gaussians, self.args.z_dim, self.args.h_dim, self.args.lr, self.args.iterations) 
...
if __name__=='__main__':
    ...
    parser.add_argument('--dataset', type=str, default='ShanghaiPM', help= 'AE or PC or PM or WP')
