...
class MDNGRU(nn.Module):
    def __init__(self, num_Gaussians, num_inputs, rnn_hidden, rnn_layers, num_outputs, dropout=0.1):
        super(MDNGRU, self).__init__()
        ...
        self.GRU=nn.GRU(num_inputs, rnn_hidden, rnn_layers, batch_first=True)
        self.linear=nn.Linear(rnn_hidden, int(num_Gaussians*(2*num_outputs+1)))
    ...
    def forward(self, x):
        ...
        raw_outputs,_=self.GRU(inputs, initial_state)  #raw_outputs (batch, L, hidden)
        hidden=self.linear(F.dropout(raw_outputs, self.dropout))[:,-1,:]  # (batch, L, output_dim), only needs the last timestep
...
class MDNGMM():
    ...
    def build(self):
        ...
        if self.args.model_type=='GRU':
            self.network=MDNGRU(self.args.num_Gaussians, self.args.num_inputs, self.args.rnn_hidden, self.args.rnn_layers, self.args.output_dim, self.args.dropout)
        self.network=self.network.to(self.device)    
        self.optimizer=torch.optim.Adam(self.network.parameters(), lr=self.args.lr, betas=(self.args.b1, self.args.b2))
    
    @property
    def model_dir(self):
        return 'MDNGMM{}_{}_{}_{}_{}_{}'.format(self.args.model_type,self.args.timesteps, self.args.pred_length,self.args.lr, self.args.iterations, self.args.dropout) 
